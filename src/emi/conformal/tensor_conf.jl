...
# Removing extreneous period
split_sent = split_sent[1: end-1]
...
# Convolution
conv_train = convolute_channel(train_tens, 2, relu)
conv_test = convolute_channel(test_tens, 2, relu)
conv_calib = convolute_channel(calib_tens, 2, relu)
...
# Saving Model
using BSON: @save
@save "emi_conf.bson" nn

using BSON: @load
BSON.@load "emi_conf.bson" nn
...
mse_acc = acc / length(conv_calib_mat[1,:])
...
setto = Vector{String}()
setto = inductive_conformal(nn, 0.40, dl_test)
...
function inductive_conformal(model, δ, dl_test, nonconf)
