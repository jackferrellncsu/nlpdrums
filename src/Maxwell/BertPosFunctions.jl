...
function embedVectorSUM(vector,embtable)
    embvec = []
    for x in vector
        if x != "/MASK/"
            push!(embvec, get(embtable, x, zeros(300)))
        end
    end
    return sum(embvec) ./ norm(sum(embvec))
end

function embedVectorNoStop(vector,embtable)
    embvec = []
    PossibleWords = []
    for x in vector
        if x != "/MASK/" &&  x ∉ [".",",","!",":","?",";"] && x ∉ stopwords(Languages.English()) && get(embtable, x, 0) != 0
            push!(embvec, embtable[x])
        end
    end
    return embvec
...
function meanNorm(P, C)
    ...
    return quantile(normz, .25)
end

function meanNormWeighted(P, C, revEmbtable, Occurence)
    normz = []
    for p in P
        if get(Occurence, revEmbtable[p], 0) != 0
        push!(normz, norm(p - C) * 1/Occurence[revEmbtable[p]])
    end
    end
    return mean(normz)
end
